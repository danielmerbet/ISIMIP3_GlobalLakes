...
variables_save=['surftemp', 'bottemp', 'thermodepth', 'strat','latentheatf', 'sensheatf', 'icethick', 'icetemp']
units = ['K', 'K', 'm', '1', 'W m-2', 'W m-2', 'm', 'K', 'm', 'm']
longnames = ['Temperature of Lake Surface Water', 'Temperature of Lake Bottom Water', 'Depth of Thermocline', 'Thermal Stratification','Latent Heat Flux at Lake-Atmosphere Interface', 'Sensible Heat Flux at Lake-Atmosphere Interface', 'Ice Thickness', 'Ice Temperature at Upper Surface', 'surface mixed layer depth', 'bottom mixed layer depth']
...
for period in get_periods(climate_path):
    ...
    for index, lake in lakes_df.iterrows():
        ...
        for variable_name in variables:
            ...
            if variable_name=='temp':
                ...
                thermoD=np.nan_to_num(thermoD)
                lake_np_decade = thermoD[int(start_decade[0]):int(end_decade[0])+1]
                #lake_np_decade = lake_np_decade.squeeze(axis=1) #remove one dimension=1 to use with values
                values_thermodepth[:, lat_pos_values-1, lon_pos_values-1] = lake_np_decade
                
                #thermal stratification
                strat=thermoD
                strat[strat > 0] = 1
                lake_np_decade = strat[int(start_decade[0]):int(end_decade[0])+1]
                #lake_np_decade = lake_np_decade.squeeze(axis=1) #remove one dimension=1 to use with values
                values_strat[:, lat_pos_values-1, lon_pos_values-1] = lake_np_decade
                #print(thermoD)
                #print(thermoD.shape)
                #print(thermoInd)
                #print(strat)
   ...
   #latent heat
    ...
    ds.to_netcdf(outpath / output_netcdf, format='NETCDF4_CLASSIC', mode='w', encoding={variables_save[6]:{'dtype':'float32'}})
    ds.close()
