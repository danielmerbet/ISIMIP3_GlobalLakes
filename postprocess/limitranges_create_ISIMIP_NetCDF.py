    ...
    for index, lake in lakes_df.iterrows():
        ...
        for variable_name in variables:
            if variable_name=='temp':
                ...
                if np.max(lake_np_decade)>50:
                    print("OJO valor mayor a 50º en surftemp en lago" + str(index+1))
                    #break
                elif np.min(lake_np_decade)<(0):
                    values_surftemp[:, lat_pos_values-1, lon_pos_values-1] = 273
                else:
                ...
                lake_np_decade = lake_np[int(start_decade[0]):int(end_decade[0])+1]
                lake_np_decade = lake_np_decade.squeeze(axis=1) #remove one dimension=1 to use with values
                if np.max(lake_np_decade)>40:
                    print("OJO valor mayor a 40º en bottemp en lago" + str(index+1))
                    #break
                elif np.min(lake_np_decade)<(0):
                    values_bottemp[:, lat_pos_values-1, lon_pos_values-1] = 273
                else:
                ...
                if variable_name=='qe':
                    if np.max(lake_np_decade)>250:
                        print("OJO valor de latent heat mayor a 250 en lago" + str(index+1))
                    elif np.min(lake_np_decade)<(-50):
                        print("OJO valor de latent heat menor a -50 en lago" + str(index+1))
                    else:
                        values_latentheatf[:, lat_pos_values-1, lon_pos_values-1] = lake_np_decade * (-1) #positive is upward

                if variable_name=='qh':
                    if np.max(lake_np_decade)>250:
                        print("OJO valor de sensible heat mayor a 250 en lago" + str(index+1))
                    elif np.min(lake_np_decade)<(-50):
                        print("OJO valor de lensible heat menor a -50 en lago" + str(index+1))
                    else:
                ...
                if variable_name=='Hice':
                    if np.max(lake_np_decade)>100:
                        print("OJO valor de ice thickness mayor a 100 en lago" + str(index+1))
                    else:
                        values_icethick[:, lat_pos_values-1, lon_pos_values-1] = lake_np_decade

                if variable_name=='Tice_surface':

                    if np.max(lake_np_decade)>2:
                        print("OJO valor de icetemp mayor a 2 en lago" + str(index+1))
                    elif np.min(lake_np_decade)<(-70):
                        print("OJO valor de icetemp menor a -70 en lago" + str(index+1))
                    else:
